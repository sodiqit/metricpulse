...
func (a *Adapter) Route() *chi.Mux {
	...
	if a.signer != nil {
		r.Use(middlewares.WithSignValidator(a.signer))
	}
...
func New(metricService metricprocessor.MetricService, storage storage.Storage, logger logger.ILogger, signer signer.Signer) *Adapter {
	return &Adapter{
		metricService,
		logger,
		storage,
